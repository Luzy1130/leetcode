...
func getHint(secret string, guess string) string {
	...
	digitMapA := make(map[byte]int)
	digitMapB := make(map[byte]int)
	i := 0
	j := 0
	for ;i < len(secret) && j < len(guess); {
		if secret[i] == guess[j] {
			numA++
		} else {
			digitMapA[secret[i]]++
	...
	for c, num := range digitMapB {
		if v, ok := digitMapA[c]; ok {
			if num > v {
				num = v
			}
			numB += num
	...
	res := strconv.Itoa(numA) + "A" + strconv.Itoa(numB) + "B"
	return res
}
// @lc code=end
